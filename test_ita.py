...
def display_document_embedding_page():
    ...
    with st.form("document_input"):
        ...
        document = st.file_uploader(
            "Knowledge Documents", type=['pdf'], help=".pdf file", accept_multiple_files= True
        )
        ...
        if document is not None:
            ...
            for file in document:
                if file.name.endswith(".pdf"):
                    ...
                    combined_content, metadata = falcon.read_pdf(file)
                  
                    # Split combined content into chunks
                    split = split + falcon.split_doc(combined_content, metadata, chunk_size, chunk_overlap)
                    
                else:
                    st.error("Check if the uploaded file is .pdf")
